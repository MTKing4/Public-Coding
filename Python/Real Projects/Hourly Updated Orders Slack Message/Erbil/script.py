...
# We inject {period_shift} and {cuttdown_hour} into the queries.
...
# ---------------- DATA HIGHLIGHTING LOGIC ----------------

def process_timestamps(df, target_dt, label_str):
    # 1. Ensure it is datetime using the generic key 'entry_date'
    ...
    # 4. Apply the Indicator and Label
...
# Rename columns to shorter names (using 'Qty' and 'Date')
...
CHANNEL_ID = "C12345678"  # Generic placeholder

def generate_platform_table(df, title):
    table = tabulate(
        df,
        headers="keys",
        tablefmt="simple", 
...
# Dispatch message to Slack
def dispatch_to_notification_service(channel_id, message):
...
msg_periodic = generate_platform_table(
    df_periodic_sync,
    "Comparative Period Activity"
)

msg_history = generate_platform_table(
    df_history,
...
if current_time_local.hour == 0:
    # If it is 00:xx, we are showing the Closing Report for Yesterday
    hour_display = "23"
else:
...
dispatch_to_notification_service(CHANNEL_ID, report_header)
